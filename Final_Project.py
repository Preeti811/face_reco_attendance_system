...
def click_photo(dregistration):
    ...
    if button['text'] == 'click_images':
        ...
        while True:
            ...
            if img_count > 70:
                ...
                image_name = str(dregistration) + \
                    "_image_{}.png".format(img_count)
    ...
    faces, faceID = fr.labels_for_training_data(
        'C:\\Users\\GAUTAM\\Desktop\\faceimage_database\\training_images')
    face_recognizer = fr.train_classifier(faces, faceID)
    face_recognizer.write(
        'C:\\Users\\GAUTAM\\Desktop\\faceimage_database\\trainingData.yml')
...
def show_frames():
    ...
    for face in faces_detected:
        ...
        label_reg, confidence = face_recognizer.predict(
...
def save():  # working appropriately
    if save_B['text'] == 'SAVE':
        ...
        if dregistration == "" or dname == "":
            ...
            con = mysql.connect(host="localhost", user="root",
                                password="1234567890", database="project_db1")
            cursor = con.cursor()
...
def delete():  # working appropriately
    if entryregi.get() == "":
        ...
        cursor = con.cursor()
        cursor.execute(
            "delete from project_db1.new_table where registration='"+entryregi.get()+"'")
        cursor.execute("commit")
...
def _update():  # working properly
    ...
    if (dregistration == "" or dname == "" or dphone == "" or dbranch == "" or demail == ""):
        ...
        con = mysql.connect(host="localhost", user="root",
                            password="123456789", database="project_db1")
        cursor = con.cursor()
        cursor.execute("update new_table set  name='"+dname+"',phone='"+dphone+"',branch='" +
                       dbranch+"', email='"+demail+"' WHERE registration='"+dregistration+"' ")
        cursor.execute("commit")
...
def _get():  # working properly
    if (entryregi.get() == ""):
        ...
        cursor = con.cursor()
        cursor.execute(
            "select * from project_db1.new_table WHERE registration='"+entryregi.get()+"'")
        rows = cursor.fetchall()
...
Bdelete = Button(gui, text="Delete", font=(
    "bold", 10), padx=10, pady=5, command=delete)

update = Button(gui, text="Update", font=("bold", 10),
                padx=10, pady=5, command=_update)
